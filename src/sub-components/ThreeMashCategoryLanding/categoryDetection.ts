import { getProductHref, type IkasProduct, type IkasProductList } from "@ikas/bp-storefront";
import {
  dentalFurnacesCategoryData,
  dentalResinsCategoryData,
  desktopScannersCategoryData,
  printerSparePartsCategoryData,
  printersCategoryData,
  systemsCategoryData,
...
const WASHING_PRODUCT_KEYS = new Set([
  tLocalized("mash-w1e-ultrasonik-yikama-cihazi", "mash-w1e-ultrasonik-yikama-cihazi"),
  "mash-w1e",
]);

const CURING_PRODUCT_KEYS = new Set([
  tLocalized("mash-c1e-uv-kurleme-cihazi", "mash-c1e-uv-kurleme-cihazi"),
  "mash-c1e",
]);
...
export function currentRouteCategoryData(): CategoryLandingData | null {
...
export function detectCurrentCategoryData(productList?: IkasProductList): CategoryLandingData | null {
  return productListCategoryData(productList) || currentRouteCategoryData();
}

export function isCurrentCategory(expectedKind: CategoryLandingData["kind"], productList?: IkasProductList): boolean {
  const detected = detectCurrentCategoryData(productList);
  // If no category could be detected at all (e.g. initial render / root), allow matching to prevent total disappearance
  if (!detected) return false;
  return detected.kind === expectedKind;
}
